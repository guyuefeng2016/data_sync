...
class CheckExistDir:
    ...
    def printUploadUsageMsg(self):
        print( 'usage:  python  %s  -e upload -t <execSeconds> -i <ftpHostIp> -u <ftpAccount> -p <ftpPassword> -l <ftpMonitorLocalSyncDir> -r <ftpUploadRemoteDir>' % sys.argv[0])
    ...
    def checkDataExists(self):
        try:
            opts, args = getopt.getopt(sys.argv[1:], 'he:t:i:u:p:l:r:x:y',
                                       ['help', 'execType=', 'execSeconds=', 'ftpHostIp=', 'ftpAccount=', 'ftpPassword=',
                                        'ftpMonitorLocalSyncDir=', 'ftpUploadRemoteDir=', 'ftpDownloadLocalDir=',
                                        'ftpDownloadRemoteDir='])
                    ...
                    try:
                        opts2, args2 = getopt.getopt(sys.argv[1:], 'he:t:i:u:p:l:r:',
                                                     ['help', 'execType=', 'execSeconds=', 'ftpHostIp=', 'ftpAccount=',
                                                      'ftpPassword=', 'ftpMonitorLocalSyncDir=', 'ftpUploadRemoteDir='])
                    except getopt.GetoptError:
                        self.printErrorMsg()
